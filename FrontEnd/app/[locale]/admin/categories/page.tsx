...
export default async function CategoryPage({
  ...
  const list = await getAll<ITreeContext>("Categories", {page, pageSize,byConfig:true,onlyActives:false});
  const res = await getFormConfigByEntityName("Categories");

  return (
    <div className="p-6">
      <h1 className="mb-4 font-semibold text-lg">لیست دسته‌بندی‌ها</h1>
      <AdminCategoryTemplate categories={list?.data.records?? []} entityFormConfig={res} />
